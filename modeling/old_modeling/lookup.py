...
def enrich_data_from_ids(master_path, files_to_process):
    ...
    # Nếu có cột 'date', chuẩn hoá và điều chỉnh năm nhỏ hơn 2015 bằng cách cộng 13 năm
    if 'date' in df_master.columns:
        try:
            df_master['date'] = pd.to_datetime(df_master['date'], errors='coerce')
            mask_year = df_master['date'].dt.year < 2015
            if mask_year.any():
                df_master.loc[mask_year, 'date'] = df_master.loc[mask_year, 'date'] + pd.DateOffset(years=13)
                print(f"ℹ️ Adjusted {mask_year.sum()} rows in 'date' by +13 years where year < 2015")
        except Exception as _:
            # nếu parse lỗi, bỏ qua điều chỉnh
            print("⚠️ Không thể parse/điều chỉnh cột 'date' trong master; bỏ qua bước đồng bộ năm.")
